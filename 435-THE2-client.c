...
// struct udp_packet is used to send data between client and server.
struct udp_packet
{
    int checksum; //int checksum is kept for noise check.
	
	short seq_num;  //short seq_num is used to inform the receiver which sequence
...
// when it's called it receives 1 packet.
				  ...
				  struct sockaddr_in* addr, int* len)
{
	...
	if(packet->seq_num == -1) // check if it is an ACK.
	{
...
int main(int argc, char* argv[])
{
	...
	establish_comm(sockfd, &servaddr, &len);
